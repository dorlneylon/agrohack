...
if __name__ == "__main__":
    file_pref = __file__[:-17]
    with open(file_pref + "illness_info.json", encoding="utf8") as f:
        illnesses = json.load(f)
        
    builder = WeatherMatrixBuilder.WeatherMatrixBuilder(file_pref)
    grad_coord_x = builder.gr1
    grad_coord_y = builder.gr2

    with open(file_pref + "cfg.json") as f:
        cfg = json.load(f)
    eps_t = cfg['eps_t']
    eps_u = cfg['eps_u']
    ...
    for t in tqdm.tqdm(ts):
        paths.append(file_pref + f"days_log/{t}.npy")
        if (os.path.exists(file_pref + f"days_log/{t}.npy")):
            continue
        builder.produce(t, data="days_log_raw")
    ans = get_probability(paths)
    save_path = file_pref[:-9] + "/storage/" + f"zalupa_{predict_time}.json"
